...
# Graphing structure rates 

```{r}

series <- c("DGS1", "DGS2", "DGS3", "DGS5", "DGS7", "DGS10", "DGS20", "DGS30")

rates <- 
  tidyquant::tq_get(series, 
                    get = "economic.data", 
                    from = "1970-01-01") %>% 
  dplyr::filter(date >= "2000-01-01") %>% 
  ggplot(aes(x = date, y = price, col = symbol)) + geom_line()

rates <- ggplotly(rates)

rates
```


# Rates graphed 2, 5, 10, 20, 30

```{r}

series <- c("DGS2","DGS5", "DGS10", "DGS20", "DGS30")
...
rates <- rates %>% dplyr::select(date, DGS2, DGS5, DGS10) %>% 
  dplyr::mutate(DGS2_DGS10_Spread = DGS10 - DGS2, 
                DGS5_DGS10_Spread = DGS10 - DGS5) %>% 
  dplyr::select(date, DGS2_DGS10_Spread,DGS5_DGS10_Spread) %>% 
  tidyr::pivot_longer(-date, names_to = "symbol", values_to = "price") 

spread_chart <- rates %>% ggplot(aes(x = date, y = price, col = symbol)) + geom_line()

spread_chart <- ggplotly(spread_chart)

spread_chart

```

# Graphing change basis points - not complete 
...
basis_point <- rates %>% 
  ggplot(aes(x = date, y = price_delta, col = symbol)) + geom_point()

basis_point <- ggplotly(basis_point)
